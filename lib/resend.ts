...
// HTML escape user-supplied fields to prevent injection
...
export async function sendConfirmation(reservation: ReservationEmail) {
  ...
  const escapedDate = escapeHtml(reservation.date)
  const escapedTime = escapeHtml(reservation.time)

  await resend.emails.send({
    from: 'noreply@cayobar.com',
    to: reservation.email,
    subject: `אישור הזמנה – ${escapedDate}`,
    html: `
      <div dir="rtl" style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 32px; background: #1A0A10; color: #F2E6D0; border-radius: 12px;">
        <h2 style="color: #C4784A; margin-bottom: 24px;">תודה ${escapedName}!</h2>
        ...
        <div style="background: #4A0E1C; padding: 20px; border-radius: 8px; margin-bottom: 16px;">
          <p style="margin: 8px 0;"><strong>תאריך:</strong> ${escapedDate}</p>
          <p style="margin: 8px 0;"><strong>שעה:</strong> ${escapedTime}</p>
